Shader "Hidden/Shader/CloudsHDRP"
{
	...
	#include "Assets/Imports/SebLague-Clouds/Scripts/Clouds/Shaders/CloudDebug.cginc"
    ...
    Varyings Vert(Attributes input)
    {
		...
		float3 viewVector = mul(_InvProjMatrix, float4(input.texcoord * 2 - 1, 0, -1));
		output.viewVector = mul(_InvViewProjMatrix, float4(viewVector, 0));
        return output;
    }
	...
	float2 squareUV(float2 texcoord) {
	...
	float4 debugDrawNoise(float2 texcoord) {

		float4 channels = 0;
		float3 samplePos = float3(texcoord.x, texcoord.y, debugNoiseSliceDepth);
    ...
    float4 CustomPostProcess(Varyings input) : SV_Target
    {
		...
		if (debugViewMode != 0) {
			...
			float y = (1 - input.texcoord.y) * height;
		...
		float nonlin_depth = SampleCameraDepth(input.texcoord);
		float depth = LinearEyeDepth(nonlin_depth, _ZBufferParams) * viewLength;
		//float depth = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, input.texcoord)) * viewLength;
		...
		//float2 samplePos;// = float3(texcoord.x, texcoord.y, debugNoiseSliceDepth);
		//float3 samplePos = float3(squareUV(input.texcoord * 3), _Time.x);
		//debugNoiseSliceDepth = _SinTime.z;
		//DetailNoiseTex.SampleLevel(samplerDetailNoiseTex, detailSamplePos, mipLevel)
		//float randomOffset = NoiseTex.SampleLevel(samplerNoiseTex, float3(squareUV(input.texcoord * .6), _Time.x), 0) *BlueNoise.SampleLevel(samplerBlueNoise, squareUV((input.texcoord + _Time / 5) * .01), 0);
		float randomOffset = BlueNoise.SampleLevel(samplerBlueNoise, squareUV(input.texcoord * 3), 0);
		if (especiallyNoisyRayOffsets)
			randomOffset += DetailNoiseTex.SampleLevel(samplerDetailNoiseTex, float3(squareUV((input.texcoord * 3) * .1), _SinTime.z / 5), 0);
		...
		float3 backgroundCol = tex2D(_MainTex, input.texcoord);
		float sun = 0;
		// do fog calculation
		if (fogDensity != 0) {
